...
void bird()
{
	...
	GLfloat mm=0.110f; GLfloat nn=.801f; GLfloat radiusmm =.01f;
	int triangleAmount = 20;
	GLfloat twicePi = 2.0f * PI;
...
{
    char text[120];
    sprintf(text, "Village natural view ",5.00,8.00);
    ...
    for(int i = 0; text[i] != '\0'; i++)
    {
        ...
        sprintf(text,"Yeasir and Sadman",5.00,8.00);
    glColor3f(0, 0, 1);
    glRasterPos2f(-20 , -25 );
    for(int i = 0; text[i] != '\0'; i++)
    {
	...
	switch (key) {
	    case  'd':
         ...
         case 'n':
            ...
            PlaySound(TEXT("Night_Sounds.wav"), NULL, SND_ASYNC);
            glutPostRedisplay();
            break;
